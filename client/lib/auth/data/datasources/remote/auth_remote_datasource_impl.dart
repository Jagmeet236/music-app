...
class AuthRemoteDatasourceImpl implements AuthRemoteDatasource {
  ...
  ResultFuture<UserModel> login({
    ...
    try {
      ...
      return Left(AppFailure('$errUnexpectedError: $e'));
    }
  }
  ...
  @override
  ResultFuture<SendOtpResponseModel> sendOtp({required String email}) async {
    if (kDebugMode) {
  ...
  ResultFuture<VerifyOtpResponseModel> verifyOtp({
    ...
    if (kDebugMode) {
      log('Verifying OTP for $email with purpose: $purpose');
    }

    try {
      final response = await _apiService.request(
        url: '${ServerConstant.serverUrl}/auth/verify-otp',
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
      ...
      if (response.statusCode == 200) {
        ...
        final model = VerifyOtpResponseModel.fromJson(decoded);
        if (model.success) {
          return Right(model);
        } else {
          return Left(AppFailure(model.message));
  ...
  ResultFuture<ResetPasswordResponseModel> resetPassword({
    ...
    if (kDebugMode) {
      log('Resetting password with provided token.');
    }

    try {
      final response = await _apiService.request(
        url: '${ServerConstant.serverUrl}/auth/reset-password',
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
      ...
      if (response.statusCode == 200) {
        final model = ResetPasswordResponseModel.fromJson(decoded);
        if (model.success) {
          return Right(model);
        } else {
          return Left(AppFailure(model.message));
