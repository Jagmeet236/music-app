...
class RecentlyPlayedViewModel extends _$RecentlyPlayedViewModel {
  ...
  FutureOr<List<SongModel>> build() async {
    ...
    ref..listen(currentSongNotifierProvider, (previous, next) {
      if (next.song != null && previous?.song?.songUrl != next.song?.songUrl) {
  ...
  Future<List<SongModel>> _loadSongs() async {
    ...
    return repository.getRecentlyPlayedSongs(user!.id!);
  }
  ...
  Future<void> addRecentlyPlayedSong(SongModel song) async {
    ...
    await repository.saveRecentlyPlayedSongs(user!.id!, updatedList);
  }
  ...
  Future<void> removeRecentlyPlayedSong(String songId) async {
    ...
    await repository.removeRecentlyPlayedSong(user!.id!, songId);
  }
}
